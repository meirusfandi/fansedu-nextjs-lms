...
function NavLink({
  ...
  return (
    ...
    >
      <span>{label}</span>
      {isActive && (
        <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-zinc-300">
          Active
        </span>
      )}
    </Link>
...
export function AdminSidebar({ currentPath = "", onLogout }: AdminSidebarProps) {
  ...
  return (
    <aside className="hidden w-64 flex-col border-r border-zinc-200 bg-white/80 px-5 py-6 backdrop-blur-sm md:flex">
      <div className="mb-8 flex items-center gap-2">
        <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-zinc-900 text-sm font-semibold text-zinc-50">
          FE
        </div>
        <div>
      ...
      <nav className="space-y-1 text-sm">
        <p className="px-2 text-xs font-medium uppercase tracking-[0.2em] text-zinc-500">
          Overview
        </p>
        ...
        <div className="mt-4 space-y-1">
          ...
          {isMasterData && (
            <div className="ml-3 mt-1 space-y-0.5 border-l border-zinc-200 pl-2">
      ...
      <div className="mt-auto space-y-3 pt-6 text-xs">
        <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-zinc-700">
